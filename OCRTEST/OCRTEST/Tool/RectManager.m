...
+ (CGRect)getEffectImageRect:(CGSize)size {
    ...
    if(size.width/size.height > size2.height/size2.width) {
        ...
        size.width = size2.height / size2.width * size.height;
        point.x = (oldW - size.width)/2;
        point.y = 0;
    }
    else {
        float oldH = size.height;
        size.height = size2.width / size2.height * size.width;
        point.x = 0;
...
+ (CGRect)getGuideFrame:(CGRect)rect {
    ...
    cardw = previewWidth*60/120;
    //if(cardw < 720) cardw = 720;
    if(previewWidth < cardw)
        cardw = previewWidth - 40;
    ...
    return CGRectMake(left+rect.origin.x, top+rect.origin.y, cardw, cardh - 40);
}
